...
# Set: fly secrets set TIMER_BOT_ID=<id> --app atd-sheet-bot
...
async def _handle_sbl_block(message: discord.Message):
    """Un-highlight a player via Timer Bot's SBL_BLOCK or SBL_VETO
    confirmation, making them available again.

    SBL_VETO fires when Timer Bot rejects a pick (e.g. repicking the player
    you were just blocked on) that this bot already highlighted, since it
    parses the raw message independently and has no visibility into Timer
    Bot's rules — the un-highlight logic to undo that is identical to a block.
    """
    ...
    for line in message.content.splitlines():
        line = line.strip()
        if not (line.startswith("SBL_BLOCK |") or line.startswith("SBL_VETO |")):
            continue
        ...
        match = find_best_match(player_raw, names, row_map, keys, key_to_name)
        if not match:
            log.info(f"[SBL] No sheet match for '{player_raw}'")
            continue

        name, row = match
        key = f"{ws.title}:{name.lower()}"
        state = get_state(channel_id)
        if key not in state["highlighted"]:
            continue
        ...
        save_state()
        log.info(f"[SBL] channel={channel_id} un-highlighted '{name}'")
...
async def on_message(message: discord.Message):
    ...
    # Steal/block declarations are Timer Bot's to referee — its SBL_BLOCK
    # confirmation (handled above) is what actually un-highlights a player,
    # so don't let the raw declaration itself be treated as a pick attempt.
    if _has_sbl_intent(content):
        return
